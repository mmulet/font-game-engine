...
from fontemon_blender_addon.fontTools.misc.loggingTools import deprecateArgument
from fontemon_blender_addon.fontTools.ttLib import TTLibError
from fontemon_blender_addon.fontTools.ttLib.sfnt import SFNTReader, SFNTWriter
import os
import logging
import itertools
...
#         pass
#         now = datetime.now().timestamp()
#         print(f"{name}: {now - self.lastTime}\nTotal: {now - self.start}\n\n")
#         self.lastTime = now
...
class TTFont(object):
	...
	def importXML(self, fileOrPath, quiet=None):
		...
		from fontemon_blender_addon.fontTools.misc import xmlReader
	...
	def _makeGlyphName(codepoint):
		from fontemon_blender_addon.fontTools import agl  # Adobe Glyph List
	...
	def getGlyphNames2(self):
		...
		"""
		from fontemon_blender_addon.fontTools.misc import textTools
	...
	def getTableData(self, tag):
		...
		if self.isLoaded(tag):
			...
			# lap = Lap(f"compile table {tag}")
			# if tag == "hhea":
				# breakpoint()
			a = self.tables[tag].compile(self)
			# lap.log("compiled")
			return a
...
class _TTGlyph(object):
	...
	def draw(self, pen):
		"""Draw the glyph onto Pen. See fontemon_blender_addon.fontTools.pens.basePen for details
		how that works.
		"""
...
class _TTGlyphGlyf(_TTGlyph):
	...
	def drawPoints(self, pen):
		"""Draw the glyph onto PointPen. See fontemon_blender_addon.fontTools.pens.pointPen
...
def getTableModule(tag):
	...
	try:
		__import__("fontemon_blender_addon.fontTools.ttLib.tables." + pyTag)
...
def registerCustomTableClass(tag, moduleName, className=None):
	...
	fontemon_blender_addon.fontTools.ttLib.tables.DefaultTable.DefaultTable
	"""
...
def getSearchRange(n, itemSize=16):
	...
	# with upstream fontemon_blender_addon.fontTools.
