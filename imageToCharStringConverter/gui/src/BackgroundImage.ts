...
export class BackgroundImage {
  ...
  setupMouseAndTouchListeners = () => {
    ...
    //   "click",
    //   ({ clientX, clientY }: MouseEvent) => {
    //     const k = Math.floor((clientY - this.y) / 64);
    //     const i = Math.floor((clientX - this.x) / 128);
    //     const kKey = this.y > 68 ? k - 1 : k
    //     this.specialFrames.get(kKey ) ?? new Map()
    //     this.specialFrames.set(kKey ) ?? new Map()
    //   },
    //   false
    // );
    document
