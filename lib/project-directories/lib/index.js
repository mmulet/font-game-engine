...
const makeDirectory = (path, makeType = false) => {
    ...
    for (const entry of entries) {
        ...
        const entryKey = entry.name.replace(/-(.)/g, (_match, letterAfterKebob) => letterAfterKebob !== "-" ? letterAfterKebob.toUpperCase() : "");
        switch (entry.name) {
            case "node_modules":
            case ".git":
            case ".vscode":
...
const getProjectRootDirectory = () => {
    const projectDirectoriesSubFolder = dirname(new URL(import.meta.url).pathname.substring(1));
...
export const directories = makeDirectory(getProjectRootDirectory().projectRootDirectory);
